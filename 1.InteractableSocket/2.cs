...
public class MyCustomMetaSnapInteractable : SnapInteractable
{
    ...
    private bool IsCorrectMatch(SnapInteractor interactor)
    {
        ...
        // Check if the text object matches this socket
        // Text_I should match with socket I
        // Text_CAN should match with socket Can
        // Text_SIGN should match with socket Sign
        if (interactorName.Contains("TEXT_"))
        {
            string textType = interactorName.Replace("TEXT_", "");
            return socketName.Contains(textType) || textType.Contains(socketName);
        }
        return false;
    }
